...
namespace schurcheb
{
    ...
    */
   static inline int SchurchebCheckMemLocation(int location)
   {
    ...
    */
   static inline void* SchurchebMallocHost(size_t size)
   {
      void *ptr = NULL;
      ptr = malloc(size);
      return ptr;
    ...
    */
   static inline void* SchurchebCallocHost(size_t length, int unitsize)
   {
      void *ptr = NULL;
    ...
    * @param [in] size size of the memory.
    */
   static inline void* SchurchebReallocHost(void *ptr, size_t size)
   {
    ...
    */
   static inline void SchurchebFreeHost(void *ptr)
   {
      free(ptr);
   }
    ...
    */
   static inline void* SchurchebMallocDevice(int size)
   {
      void        *ptr  = NULL;
#ifdef SCHURCHEB_CUDA
      SCHURCHEB_CUDA_CALL( cudaMalloc( &ptr, size) );
#else
    ...
    * @return  Return the pointer to the memory.
    */
   static inline void* SchurchebCallocDevice(size_t length, int unitsize)
   {
      void        *ptr = NULL;
#ifdef SCHURCHEB_CUDA
      ...
      SCHURCHEB_CUDA_CALL( cudaMalloc( &ptr, size) );
      SCHURCHEB_CUDA_CALL( cudaMemset( ptr, 0, size) );
#else
      cout<<"Error: attempt to allocate device memory with the host-only version."<<endl;
      exit(0);
#endif
    ...
    * @param [in] ptr the pointer to be freed
    */
   static inline void SchurchebFreeDevice(void *ptr)
   {
#ifdef SCHURCHEB_CUDA
      SCHURCHEB_CUDA_CALL( cudaFree( ptr) );
#else
    ...
    * @return  Return the pointer to the memory.
    */
   static inline void* SchurchebMallocUnified(size_t size)
   {
      void        *ptr  = NULL;
#ifdef SCHURCHEB_CUDA
      SCHURCHEB_CUDA_CALL( cudaMallocManaged( &ptr, size) );
#else
    ...
    * @return  Return the pointer to the memory.
    */
   static inline void* SchurchebCallocUnified(size_t length, int unitsize)
   {
      void        *ptr = NULL;
#ifdef SCHURCHEB_CUDA
      size_t size = length * unitsize;
    ...
    * @param [in] ptr the pointer to be freed
    */
   static inline void SchurchebFreeUnified(void *ptr)
   {
#ifdef SCHURCHEB_CUDA
      SCHURCHEB_CUDA_CALL( cudaFree( ptr) );
#else
    ...
    * @return  Return the pointer to the memory.
    */
   static inline void* SchurchebMallocPinned(size_t size)
   {
      void        *ptr  = NULL;
#ifdef SCHURCHEB_CUDA
      SCHURCHEB_CUDA_CALL( cudaMallocHost( &ptr, size) );
#else
    ...
    * @return  Return the pointer to the memory.
    */
   static inline void* SchurchebCallocPinned(size_t length, int unitsize)
   {
      void        *ptr = NULL;
#ifdef SCHURCHEB_CUDA
      size_t size = length * unitsize;
    ...
    * @param [in] ptr the pointer to be freed
    */
   static inline void SchurchebFreePinned(void *ptr)
   {
#ifdef SCHURCHEB_CUDA
      SCHURCHEB_CUDA_CALL( cudaFreeHost( ptr) );
#else
    ...
    * @return  Return the pointer to the memory.
    */
   static inline void* SchurchebMallocWrapper(size_t size, int location)
   {
      if (size == 0)
      {
         return NULL;
      ...
      location       = SchurchebCheckMemLocation(location);
      ...
      switch (location)
      {
         ...
         {
            /* memroy on the host */
            ptr = SchurchebMallocHost(size);
            break;
         }
         case kMemoryDevice:
         {
            /* memory on the device */
            ptr = SchurchebMallocDevice(size);
            break;
         }
         case kMemoryUnified:
         {
            /* shared memory */
            ptr = SchurchebMallocUnified(size);
            break;
         }
         case kMemoryPinned:
         {
            /* page-locked memory */
            ptr = SchurchebMallocPinned(size);
            break;
         }
         default:
         {
    ...
    * @return  Return the pointer to the memory.
    */
   static inline void* SchurchebCallocWrapper(size_t length, int unitsize, int location)
   {
      ...
      switch (location)
      {
         ...
         {
            /* memroy on the host */
            ptr = SchurchebCallocHost(length, unitsize);
            break;
         }
         case kMemoryDevice:
         {
            /* device memory */
            ptr = SchurchebCallocDevice(length, unitsize);
            break;
         }
         case kMemoryUnified:
         {
            /* shared memory */
            ptr = SchurchebCallocUnified(length, unitsize);
            break;
         }
         case kMemoryPinned:
         {
            /* page-locked memory */
            ptr = SchurchebCallocPinned(length, unitsize);
            break;
         }
         default:
         {
    ...
    */
   static inline void SchurchebMemcpyWrapper(void *ptr_to, const void *ptr_from, size_t size, int loc_to, int loc_from)
   {
      if (size == 0)
      {
         return;
      ...
      loc_to         = SchurchebCheckMemLocation(loc_to);
      loc_from       = SchurchebCheckMemLocation(loc_from);
    ...
    * @param [in] location Location of the memory.
    */
   static inline void SchurchebFreeWrapper(void *ptr, int location)
   {
      if (!ptr)
      {
         return;
      ...
      switch (location)
      {
         ...
         {
            //memroy on CPU
            SchurchebFreeHost(ptr);
            break;
         }
         case kMemoryDevice:
         {
            //memory on GPU
            SchurchebFreeDevice(ptr);
            break;
         }
         case kMemoryUnified:
         {
            //memory that is shared by CPU and GPU
            SchurchebFreeUnified(ptr);
            break;
         }
         case kMemoryPinned:
         {
            //memory on CPU that is page-locked
            SchurchebFreePinned(ptr);
            break;
         }
         default :
         {
    ...
    * @return  Return the pointer to the memory.
    */
   static inline void* SchurchebReallocWrapper(void* ptr, size_t current_size, size_t new_size, int location)
   {
      SCHURCHEB_CHKERR(new_size < 0);
      SCHURCHEB_CHKERR(current_size < 0);
    
      if (new_size == 0)
      {
         SchurchebFreeWrapper(ptr, location);
         return NULL;
      }
      //check location
      location       = SchurchebCheckMemLocation(location);
      ...
      switch (location)
      {
         ...
         {
            /* memroy on the host */
            ptr = SchurchebReallocHost(ptr, new_size);
            break;
         }
         case kMemoryDevice:
         case kMemoryUnified:
         case kMemoryPinned:
         {
            void *ptr2 = SchurchebMallocWrapper(new_size, location);
            if( new_size >= current_size)
            {
               SchurchebMemcpyWrapper( ptr2, ptr, current_size, location, location);
            }
            else
            {
               SchurchebMemcpyWrapper( ptr2, ptr, new_size, location, location);
            }
            SchurchebFree(ptr, location);
            ptr = ptr2;
            break;
         }
         default:
 ...
 * - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
   ...
   {
      return SchurchebMallocWrapper( length, location);
   }
   
   void* SchurchebCalloc( size_t length, int location)
   {
      return SchurchebCallocWrapper( length, 1, location);
   }
   
   void* SchurchebRealloc( void* ptr, size_t current_length, size_t new_length, int location)
   {
      return SchurchebReallocWrapper( (void*) ptr, current_length, new_length, location);
   }
   
   void SchurchebMemcpy( void* ptr_to, const void* ptr_from, size_t length, int loc_to, int loc_from)
   {
      SchurchebMemcpyWrapper( (void*) ptr_to, (void*) ptr_from, length, loc_to, loc_from);
   }

...
namespace schurcheb
{
   ...
   int ParallelVectorClass<T>::Setup(int n_local, parallel_log &parlog)
   {
      ...
      SCHURCHEB_long n_start, n_global;
      SchurchebNLocalToNGlobal( n_local, n_start, n_global, comm);
   ...
   template <typename T>
   int ParallelVectorClass<T>::Setup(int n_local, SCHURCHEB_long n_start, SCHURCHEB_long n_global, parallel_log &parlog)
   {
      return Setup( n_local, n_start, n_global, n_local, this->GetDataLocation(), false, parlog);
   }
   template int ParallelVectorClass<float>::Setup(int n_local, SCHURCHEB_long n_start, SCHURCHEB_long n_global, parallel_log &parlog);
   template int ParallelVectorClass<double>::Setup(int n_local, SCHURCHEB_long n_start, SCHURCHEB_long n_global, parallel_log &parlog);
   template int ParallelVectorClass<complexs>::Setup(int n_local, SCHURCHEB_long n_start, SCHURCHEB_long n_global, parallel_log &parlog);
   template int ParallelVectorClass<complexd>::Setup(int n_local, SCHURCHEB_long n_start, SCHURCHEB_long n_global, parallel_log &parlog);
   ...
   template <typename T>
   int ParallelVectorClass<T>::Setup(int n_local, SCHURCHEB_long n_start, SCHURCHEB_long n_global, bool setzero, parallel_log &parlog)
   {
      return Setup( n_local, n_start, n_global, n_local, this->GetDataLocation(), setzero, parlog);
   }
   template int ParallelVectorClass<float>::Setup(int n_local, SCHURCHEB_long n_start, SCHURCHEB_long n_global, bool setzero, parallel_log &parlog);
   template int ParallelVectorClass<double>::Setup(int n_local, SCHURCHEB_long n_start, SCHURCHEB_long n_global, bool setzero, parallel_log &parlog);
   template int ParallelVectorClass<complexs>::Setup(int n_local, SCHURCHEB_long n_start, SCHURCHEB_long n_global, bool setzero, parallel_log &parlog);
   template int ParallelVectorClass<complexd>::Setup(int n_local, SCHURCHEB_long n_start, SCHURCHEB_long n_global, bool setzero, parallel_log &parlog);
   ...
   template <typename T>
   int ParallelVectorClass<T>::Setup(int n_local, SCHURCHEB_long n_start, SCHURCHEB_long n_global, int location, bool setzero, parallel_log &parlog)
   {
      return Setup( n_local, n_start, n_global, n_local, location, setzero, parlog);
   }
   template int ParallelVectorClass<float>::Setup(int n_local, SCHURCHEB_long n_start, SCHURCHEB_long n_global, int location, bool setzero, parallel_log &parlog);
   template int ParallelVectorClass<double>::Setup(int n_local, SCHURCHEB_long n_start, SCHURCHEB_long n_global, int location, bool setzero, parallel_log &parlog);
   template int ParallelVectorClass<complexs>::Setup(int n_local, SCHURCHEB_long n_start, SCHURCHEB_long n_global, int location, bool setzero, parallel_log &parlog);
   template int ParallelVectorClass<complexd>::Setup(int n_local, SCHURCHEB_long n_start, SCHURCHEB_long n_global, int location, bool setzero, parallel_log &parlog);

   template <typename T>
   int ParallelVectorClass<T>::Setup(int n_local, SCHURCHEB_long n_start, SCHURCHEB_long n_global, int reserve, int location, bool setzero, parallel_log &parlog)
   {
      SCHURCHEB_CHKERR(n_local < 0);
      SCHURCHEB_CHKERR(n_local > reserve);
   ...
   template int ParallelVectorClass<float>::Setup(int n_local, SCHURCHEB_long n_start, SCHURCHEB_long n_global, int reserve, int location, bool setzero, parallel_log &parlog);
   template int ParallelVectorClass<double>::Setup(int n_local, SCHURCHEB_long n_start, SCHURCHEB_long n_global, int reserve, int location, bool setzero, parallel_log &parlog);
   template int ParallelVectorClass<complexs>::Setup(int n_local, SCHURCHEB_long n_start, SCHURCHEB_long n_global, int reserve, int location, bool setzero, parallel_log &parlog);
   template int ParallelVectorClass<complexd>::Setup(int n_local, SCHURCHEB_long n_start, SCHURCHEB_long n_global, int reserve, int location, bool setzero, parallel_log &parlog);
   ...
   template <typename T>
   SCHURCHEB_long ParallelVectorClass<T>::GetLengthGlobal() const
   {
      return this->_n_global;
   }
   template SCHURCHEB_long ParallelVectorClass<float>::GetLengthGlobal() const;
   template SCHURCHEB_long ParallelVectorClass<double>::GetLengthGlobal() const;
   template SCHURCHEB_long ParallelVectorClass<complexs>::GetLengthGlobal() const;
   template SCHURCHEB_long ParallelVectorClass<complexd>::GetLengthGlobal() const;
   
   template <typename T>
   SCHURCHEB_long ParallelVectorClass<T>::GetStartGlobal() const
   {
      return this->_n_start;
   }
   template SCHURCHEB_long ParallelVectorClass<float>::GetStartGlobal() const;
   template SCHURCHEB_long ParallelVectorClass<double>::GetStartGlobal() const;
   template SCHURCHEB_long ParallelVectorClass<complexs>::GetStartGlobal() const;
   template SCHURCHEB_long ParallelVectorClass<complexd>::GetStartGlobal() const;
   ...
   int ParallelVectorClass<T>::WriteToDisk( const char *datafilename)
   {
      ...
      {
         SCHURCHEB_ERROR("Can only write host data.");
      }
   ...
   int ParallelVectorClass<T>::NormInf( float &norm)
   {
      ...
      SchurchebMpiAllreduce( &local_norm, &norm, 1, MPI_MAX, *(this->_comm));
   ...
   {
      SCHURCHEB_CHKERR( this->GetLengthLocal() != y.GetLengthLocal() || this->GetLengthGlobal() != y.GetLengthGlobal() );
   ...
   int ParallelVectorClass<T>::ReadFromSingleMMFile(const char *vecfile, int idxin)
   {
      ...
      IntVectorClass<SCHURCHEB_long>   nstarts;
      
      /* only one MPI rank, do as sequential */
      if(np == 1)
      {
         err = global_vec.ReadFromMMFile( vecfile, idxin); SCHURCHEB_CHKERR(err);
         
         SCHURCHEB_CHKERR( this->_n_global != (SCHURCHEB_long)(global_vec.GetLengthLocal()));
      ...
      SCHURCHEB_MPI_CALL( SchurchebMpiGather( &(this->_n_start), 1, nstarts.GetData(), 0, comm) );
      ...
      if(myid == 0)
      {
         ...
         for(i = 1 ; i < np ; i ++)
         {
            SCHURCHEB_MPI_CALL(SchurchebMpiSend( global_vec.GetData()+nstarts[i], nstarts[i+1]-nstarts[i], i, 0, comm));
         }
         SCHURCHEB_MEMCPY( this->_data_vec.GetData(), global_vec.GetData(), this->GetLengthLocal(), kMemoryHost, kMemoryHost, T);
      }
      else
      {
         SCHURCHEB_MPI_CALL(SchurchebMpiRecv( this->_data_vec.GetData(), this->GetLengthLocal(), 0, 0, comm, MPI_STATUS_IGNORE));
      }

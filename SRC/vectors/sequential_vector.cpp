...
namespace schurcheb
{
   ...
   int SequentialVectorClass<T>::Clear()
   {
      ...
      if( this->_cusparse_vec )
      {
         ...
         SCHURCHEB_CUSPARSE_CALL( (cusparseDestroyDnVec( this->_cusparse_vec )) );
   ...
   template <typename T>
   SCHURCHEB_long SequentialVectorClass<T>::GetLengthGlobal() const
   {
      return (SCHURCHEB_long)(this->_length);
   }
   template SCHURCHEB_long SequentialVectorClass<float>::GetLengthGlobal() const;
   template SCHURCHEB_long SequentialVectorClass<double>::GetLengthGlobal() const;
   template SCHURCHEB_long SequentialVectorClass<complexs>::GetLengthGlobal() const;
   template SCHURCHEB_long SequentialVectorClass<complexd>::GetLengthGlobal() const;
   
   template <typename T>
   SCHURCHEB_long SequentialVectorClass<T>::GetStartGlobal() const
   {
      return 0;
   }
   template SCHURCHEB_long SequentialVectorClass<float>::GetStartGlobal() const;
   template SCHURCHEB_long SequentialVectorClass<double>::GetStartGlobal() const;
   template SCHURCHEB_long SequentialVectorClass<complexs>::GetStartGlobal() const;
   template SCHURCHEB_long SequentialVectorClass<complexd>::GetStartGlobal() const;
...
#pragma omp parallel for private(i) SCHURCHEB_OPENMP_SCHEDULE_DEFAULT
   ...
   int SequentialVectorClass<T>::Rand()
   {
      ...
      for(i = 0 ; i < this->_length ; i ++)
      {
         schurcheb::SchurchebValueRandHost(this->_data[i]);
      }
   ...
   int SequentialVectorClass<T>::PlotAbsGnuPlot( const char *datafilename, int conditiona, int conditionb, bool logx, bool logy, int pttype)
   {
      ...
      {
         SCHURCHEB_ERROR("Plot Abs only works on host.");
      }
   ...
   int SequentialVectorClass<T>::NormInf( float &norm)
   {
      ...
      for(i = 0 ; i < length ; i ++)
      {
         norm_temp = SchurchebAbs(this->_data[i]);
   ...
   int SequentialVectorClass<T>::Dot( const VectorClass<T> &y, T &t) const
   {
      ...
      SCHURCHEB_CHKERR( this->_length != y.GetLengthLocal() || this->_length != y.GetLengthGlobal() );

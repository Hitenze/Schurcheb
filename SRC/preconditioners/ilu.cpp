...
namespace schurcheb
{
   ...
   int IluClass<MatrixType, VectorType, DataType>::Setup( VectorType &x, VectorType &rhs)
   {
      ...
      if(!this->_matrix)
      {
         SCHURCHEB_ERROR("Call ILU without matrix.");
      ...
      switch(this->_option)
      {
         case kIluOptionILUT:
         {
            ...
            {
               SCHURCHEB_WARNING("ILUT only works on the host memory. Moving data to the host.");
         ...
         case kIluOptionILUK:
         {
            ...
            if(this->_matrix->GetDataLocation() == kMemoryDevice)
            {
               SCHURCHEB_WARNING("ILUK only works on the host memory. Moving data to the host.");
               this->_matrix->MoveData(kMemoryHost);
            }
         ...
         {
            /* Invalid option */
            SCHURCHEB_ERROR("Invalid ILU option.");
      ...
      if( (this->_option == kIluOptionILUT || this->_option == kIluOptionILUK)
         && this->_omp_option == kIluOpenMPLevelScheduling && SchurchebGetOpenmpGlobalMaxNumThreads() > 1)
      {
   ...
   int IluClass<MatrixType, VectorType, DataType>::Setup(MatrixType &L_mat, VectorType &D_vec, MatrixType &U_mat, int *pperm, int *qperm)
   {
      ...
      if(this->_ready)
      {
         SCHURCHEB_WARNING("Set user LDU failed.");
         return SCHURCHEB_SUCCESS;
      }
      ...
      {
         SCHURCHEB_ERROR("Setup ILU with L, D, and U, must all be on the host memory.");
         return SCHURCHEB_ERROR_INVALED_PARAM;
      }
      ...
      if(pperm)
      {
         ...
         if(qperm)
         {
            ...
            SCHURCHEB_MEMCPY(this->_row_perm_vec.GetData(), pperm, this->_n, kMemoryHost, kMemoryHost, int );
            SCHURCHEB_MEMCPY(this->_col_perm_vec.GetData(), qperm, this->_n, kMemoryHost, kMemoryHost, int );
         }
         else
         {
            this->_row_perm_vec.Setup(this->_n);
...
#ifdef SCHURCHEB_OPENMP
      if(this->_omp_option == kIluOpenMPLevelScheduling && SchurchebGetOpenmpGlobalMaxNumThreads() > 1)
      {
         switch(this->_omp_option)
         {
            case kIluOpenMPLevelScheduling:
   ...
   int IluClass<MatrixType, VectorType, DataType>::SetupPermutation( VectorType &x, VectorType &rhs)
   {
      ...
      SCHURCHEB_CHKERR(this->_matrix->GetNumRowsLocal() != this->_matrix->GetNumColsLocal())
      ...
      switch(this->_perm_option)
      {
         ...
         case kIluReorderingRcm:
         {
            if(this->_matrix->GetDataLocation() == kMemoryDevice)
            {
               SCHURCHEB_WARNING("ILU reordering only works on the host memory. Moving data to the host.");
               this->_matrix->MoveData(kMemoryHost);
            }
         ...
         {
            SCHURCHEB_ERROR("Ddpq ordering not supported yet.");
            return SCHURCHEB_ERROR_INVALED_OPTION;
            break;
         }
         ...
         {
            /* Invalid option */
            SCHURCHEB_ERROR("Invalid ILU permutation option.");
            return SCHURCHEB_ERROR_INVALED_OPTION;
         }
      }
   ...
   int IluClass<MatrixType, VectorType, DataType>::ApplyShift( ComplexValueClass<T> &w, int nz, T norm)
   {
      ...
      if(this->_complex_shift < 0)
      {
         ...
         shf = (norm - T(2.0)*SchurchebAbs(w))/(T)nz;
   ...
   int IluClass<MatrixType, VectorType, DataType>::Qsplit(T *a, int *ind, int n, int Ncut)
   {
      ...
      |
      |     SchurchebAbs(a[i]) >= SchurchebAbs(a[Ncut-1]) for i < Ncut-1 and
      |     SchurchebAbs(a[i]) <= SchurchebAbs(a[Ncut-1]) for i > Ncut-1
      |
      |     ind[0 : (n-1)] is an integer array permuted in the same way as a.
      |---------------------------------------------------------------------*/
   
      typedef typename std::conditional<SchurchebIsDoublePrecision<DataType>::value, double, float>::type RealDataType;
   
      RealDataType   tmp, SchurchebAbskey;
      ...
      {
         mid = first;
         SchurchebAbskey = SchurchebAbs(a[mid]);
         for (j=first+1; j<=last; j++) 
         {
            if (SchurchebAbs(a[j]) > SchurchebAbskey) 
            {
               mid = mid+1;
               tmp = a[mid];
               itmp = ind[mid];
   ...
   int IluClass<MatrixType, VectorType, DataType>::BuildLevels()
   {
      ...
      if(!(this->_ready))
      {
         SCHURCHEB_ERROR("ILU level scheduling without setup.");
      ...
      {
         SCHURCHEB_PRINT("Using level scheduling ILU\n");
      }
      ...
      for( i = 0; i < this->_n; i++ )
      {
         ...
         for(j = k1 ; j < k2; j++) 
         {
             ...
             */
            level = SchurchebMax(level, levels[L_j[j]]);
         }
         /* now, level is the highest level that this node depends on */
         levels[i] = level + 1;
         max_level = SchurchebMax(max_level, levels[i]);
      }
      ...
      if(this->_print_option > 0)
      {
         SCHURCHEB_PRINT("L part %d/%d levels\n",max_level,this->_n);
      }
      ...
      for( i = 0; i < max_level; i++ )
      {
         if(this->_level_ptr_l[i+1] - this->_level_ptr_l[i] > SCHURCHEB_global::_openmp_min_loopsize)
         {
      ...
      for( i = this->_n-1; i >= 0; i-- ) 
      {
         ...
         for( j = k1 ; j < k2; j++) 
         {
             ...
             * note that in the solve phase, perm is apply to x, no need to consider in the level scheduling
             */
            level = SchurchebMax(level, levels[U_j[j]]);
         }
         /* now, level is the highest level that this node depends on */
         levels[i] = level + 1; 
         max_level = SchurchebMax(max_level, levels[i]);   
      ...
      if(this->_print_option > 0)
      {
         SCHURCHEB_PRINT("U part %d/%d levels\n",max_level,this->_n);
      }
      /* first sum the size on each level */
      for( i = 0; i < this->_n; i++ )
      {
      ...
      for( i = 0; i < max_level; i++ )
      {
         if(this->_level_ptr_u[i+1] - this->_level_ptr_u[i] > SCHURCHEB_global::_openmp_min_loopsize)
         {
   ...
   int IluClass<MatrixType, VectorType, DataType>::BuildPoly()
   {
      ...
      if(this->_print_option > 0)
      {
         SCHURCHEB_PRINT("Using polynomia ILU solve\n");
      }
...
#pragma omp parallel for private(i) SCHURCHEB_OPENMP_SCHEDULE_DEFAULT
         ...
         default:
         {
            SCHURCHEB_ERROR("Invalid ILU polynomial solve option.");
            return SCHURCHEB_ERROR_INVALED_OPTION;
         }
      }
...
#pragma omp for SCHURCHEB_OPENMP_SCHEDULE_DEFAULT
         ...
         default:
         {
            SCHURCHEB_ERROR("Unknown ILU openmp option.");
            return SCHURCHEB_ERROR_INVALED_OPTION;
         }
      }
   ...
   int IluClass<MatrixType, VectorType, DataType>::SetupILUT( VectorType &x, VectorType &rhs)
   {
      ...
      SCHURCHEB_CHKERR(n != this->_matrix->GetNumColsLocal())
      ...
      if(this->_row_perm_vec.GetLengthLocal() > 0)
      {
         ...
         SCHURCHEB_MALLOC(rqperm, n, kMemoryHost, int);
#ifdef SCHURCHEB_OPENMP
#pragma omp parallel for private(i) SCHURCHEB_OPENMP_SCHEDULE_DEFAULT
#endif
         for(i = 0 ; i < n ; i++)
      ...
      init_nnz = SchurchebMin((int)(n + this->_matrix->GetNumNonzeros() / 2.0), (int)(n * lfil));
      nnzL_max = init_nnz;
      nnzU_max = init_nnz;
      ...
      SCHURCHEB_MALLOC(L_j, init_nnz, kMemoryHost, int);
      SCHURCHEB_MALLOC(U_j, init_nnz, kMemoryHost, int);
      SCHURCHEB_MALLOC(L_a, init_nnz, kMemoryHost, T);
      SCHURCHEB_MALLOC(U_a, init_nnz, kMemoryHost, T);
      
      /* working buffers */
      SCHURCHEB_MALLOC(iw, n, kMemoryHost, int);
      SCHURCHEB_MALLOC(jbuf, n, kMemoryHost, int);
      SCHURCHEB_MALLOC(wn, n, kMemoryHost, RealDataType);
      SCHURCHEB_MALLOC(w, n, kMemoryHost, T);
...
/*---------- unpack L-part and U-part of column of A in arrays w */
         ...
         if( tnorm == 0.0 ) 
         {
            SCHURCHEB_WARNING("ILUT zero row encountered.");
            return SCHURCHEB_ERROR_ILU_EMPTY_ROW;
         }
...
/*---------- determine smallest column index */
   ...
   /*---------- if fill-in element is small then disregard */
               if( SchurchebAbs( lxu ) < tolnorm && jpos == -1 ) 
               {  
                  if(_modified)
                  {
                     if(standard_milu)
   ...
   /*---------- dealing with upper part */
                  if( jpos == -1 && SchurchebAbs(lxu) > tolnorm) 
                  {
   /*---------- this is a fill-in element */
                     lenu++;
                     upos = ii + lenu;
   ...
   /*---------- case when diagonal is zero */
         //if( SchurchebAbs(w[ii]) < 1e-06) 
         //{
         //   w[ii] = 1e-06;
         //   SCHURCHEB_WARNING("ILUT small diagonal encountered.");
         //}
         //D[ii] = T(1.0) / w[ii]; 
   ...
   //--------------------    len = min( lenl, lfil );
         ...
         if( nnzL > nnzL_max ) 
         {
            ...
            {
               nnzL_max2 = SchurchebMax(nnzL_max+1, (int)(nnzL_max*schurcheb::SCHURCHEB_global::_expand_fact));
            }
            SCHURCHEB_REALLOC(L_j, nnzL_max, nnzL_max2, kMemoryHost, int);
            SCHURCHEB_REALLOC(L_a, nnzL_max, nnzL_max2, kMemoryHost, T);
            nnzL_max = nnzL_max2;
         }
         ja = L_j+L_i[ii];
         ma = L_a+L_i[ii];
...
/*---------- len = min(lfil, lenu) */
         ...
         if( nnzU > nnzU_max ) 
         {
            ...
            {
               nnzU_max2 = SchurchebMax(nnzU_max+1, (int)(nnzU_max*schurcheb::SCHURCHEB_global::_expand_fact));
            }
            SCHURCHEB_REALLOC(U_j, nnzU_max, nnzU_max2, kMemoryHost, int);
            SCHURCHEB_REALLOC(U_a, nnzU_max, nnzU_max2, kMemoryHost, T);
            nnzU_max = nnzU_max2;
         }
         ja = U_j+U_i[ii];
         ma = U_a+U_i[ii];
      ...
      SCHURCHEB_FREE( w, kMemoryHost);
      SCHURCHEB_FREE( iw, kMemoryHost);
      SCHURCHEB_FREE( jbuf, kMemoryHost);
      SCHURCHEB_FREE( wn, kMemoryHost);
      
      if(this->_row_perm_vec.GetLengthLocal() > 0)
      {
         SCHURCHEB_FREE(rqperm, kMemoryHost);
      }
      unitperm.Clear();
   ...
   int IluClass<MatrixType, VectorType, DataType>::SetupILUK( VectorType &x, VectorType &rhs)
   {
      ...
      if(this->_print_option>0)
      {
         SCHURCHEB_PRINT("Start ILUK\n");
         SCHURCHEB_PRINT("Level of fill: %d\n", this->_fill_level);
      }
...
#pragma omp parallel for private(i) SCHURCHEB_OPENMP_SCHEDULE_DEFAULT
      ...
      SCHURCHEB_CALLOC( iw, 3*n, kMemoryHost, int);
      L_i[0] = U_i[0] = 0;
      ...
      if(L_i[n])
      {
         SCHURCHEB_CALLOC( L_a, L_i[n], kMemoryHost, T);
      }
      if(U_i[n])
      {
         SCHURCHEB_CALLOC( U_a, U_i[n], kMemoryHost, T);
      }
      ...
      for( ii = 0; ii < n; ii++ )
      {
         ...
         if(SchurchebAbs(D[ii]) < 1e-06)
         {
            if(!small_warning)
            {
               SCHURCHEB_WARNING("ILUK small diagonal encountered.");
               small_warning = true;
            }
            D[ii] = T(1e-06);
         }
   ...
   int IluClass<MatrixType, VectorType, DataType>::SetupILUKSymbolic( int n, int *A_i, int *A_j, int lfil, int *perm, int *qperm, int *rqperm, int *iw, int *L_i, int *U_i, int **L_jp, int **U_jp)
   {
      ...
      SCHURCHEB_CALLOC( temp_L_j, capacity_L, kMemoryHost, int);
      SCHURCHEB_CALLOC( temp_U_j, capacity_U, kMemoryHost, int);
      SCHURCHEB_CALLOC( u_levels, capacity_U, kMemoryHost, int);
      ...
      for(ii = 0 ; ii < n ; ii ++)
      {
         ...
         while(lenh > 0)
         {
            ...
            for(j = U_i[k] ; j < ku ; j ++)
            {
               ...
               {
                  iLev[icol] = SchurchebMin(lev, iLev[icol]);
               }
         ...
         if(lenl > 0)
         {
            ...
            if(ctrL + lenl > capacity_L)
            {
               ...
               {
                  capacity_L = capacity_L * SCHURCHEB_global::_expand_fact + 1;
               }
               SCHURCHEB_REALLOC( temp_L_j, tmp, capacity_L, kMemoryHost, int);
            }
         ...
         if(k > 0)
         {
            ...
            if(ctrU + k > capacity_U)
            {
               ...
               {
                  capacity_U = capacity_U * SCHURCHEB_global::_expand_fact + 1;
               }
               SCHURCHEB_REALLOC( temp_U_j, tmp, capacity_U, kMemoryHost, int);
               SCHURCHEB_REALLOC( u_levels, tmp, capacity_U, kMemoryHost, int);
            }
            
            SCHURCHEB_MEMCPY( temp_U_j+ctrU, iL+ii, k, kMemoryHost, kMemoryHost, int);
            SCHURCHEB_MEMCPY( u_levels+ctrU, iLev+ii, k, kMemoryHost, kMemoryHost, int);
      ...
      SCHURCHEB_FREE( u_levels, kMemoryHost);
   ...
   int IluClass<MatrixType, VectorType, DataType>::SetupPartialILUT(VectorType &x, VectorType &rhs)
   {
      ...
      if(n > 0)
      {
         initial_alloc = SchurchebMin(nLU + ceil((nnz_A / 2.0) * nLU / n), (double)nLU * this->_max_row_nnz);
      }
      capacity_L = initial_alloc;
      capacity_U = initial_alloc;
      
      this->_D.Setup(nLU);
      SCHURCHEB_MALLOC( L_i, n+1, kMemoryHost, int);
      SCHURCHEB_MALLOC( U_i, nLU+1, kMemoryHost, int);
      SCHURCHEB_MALLOC( L_j, capacity_L, kMemoryHost, int);
      SCHURCHEB_MALLOC( U_j, capacity_U, kMemoryHost, int);
      SCHURCHEB_MALLOC( L_data, capacity_L, kMemoryHost, T);
      SCHURCHEB_MALLOC( U_data, capacity_U, kMemoryHost, T);
      
      ctrL = ctrU = ctrUB = 0;

      ctrS = 0;
      SCHURCHEB_MALLOC( S_i, m+1, kMemoryHost, int);
      S_i[0] = 0;
      /* only setup S part when n > nLU */
      if(m > 0)
      {
         capacity_S = SchurchebMin(m + ceil((nnz_A / 2.0) * m / n), (double)m * this->_max_row_nnz_s);
         SCHURCHEB_MALLOC( S_j, capacity_S, kMemoryHost, int);
         SCHURCHEB_MALLOC( S_data, capacity_S, kMemoryHost, T);
      }
      ...
      for(ii = 0 ; ii < nLU ; ii ++)
      {
         ...
         for(j = k1 ; j < k2 ; j ++)
         {
            inorm += SchurchebAbs(A_data[j]);
         }
         if(inorm == 0.0)
         {
            SCHURCHEB_WARNING("Empty row in partial ILUT.");
         }
         ...
         while(lenhll > 0)
         {
            ...
            for(j = U_i[jrow] ; j < ku ; j ++)
            {
               ...
               if( icol == -1 && ( (col < nLU && SchurchebAbs(lxu) < itolb) || (col >= nLU && SchurchebAbs(lxu) < itolef) ) )
               {
                  continue;
               }
               if(icol == -1)
         ...
         if(lenl > 0)
         {
            ...
            if(ctrL + lenl > capacity_L)
            {
               ...
               SCHURCHEB_REALLOC(L_j, tmp, capacity_L, kMemoryHost, int);
               SCHURCHEB_REALLOC(L_data, tmp, capacity_L, kMemoryHost, T);
            }
            ctrL += lenl;
            /* copy large data in */
         ...
         if(lenhu > 0)
         {
            ...
            if(ctrU + lenhu > capacity_U)
            {
               ...
               SCHURCHEB_REALLOC(U_j, tmp, capacity_U, kMemoryHost, int);
               SCHURCHEB_REALLOC(U_data, tmp, capacity_U, kMemoryHost, T);
            }
            ctrU += lenhu;
            /* copy large data in */
      ...
      for(ii = nLU ; ii < n ; ii ++)
      {
         ...
         while(lenhll > 0)
         {
            ...
            for(j = U_i[jrow] ; j < ku ; j ++)
            {
               ...
               lxu = - dpiv*U_data[j];
               /* we don't want to fill small number to empty place */
               if(icol == -1 && ( (col < nLU && SchurchebAbs(lxu) < itolef) || ( col >= nLU && SchurchebAbs(lxu) < itols ) ) )
               {
                  continue;
               }
               if(icol == -1)
         ...
         if(ctrS + lenhu + 1 > capacity_S)
         {
            ...
            {
               capacity_S = capacity_S * SCHURCHEB_global::_expand_fact + 1;
            }
            SCHURCHEB_REALLOC(S_j, tmp, capacity_S, kMemoryHost, int);
            SCHURCHEB_REALLOC(S_data, tmp, capacity_S, kMemoryHost, T);
         }
      ...
      SCHURCHEB_MEMCPY( this->_L.GetI(), L_i, nLU+1, kMemoryHost, kMemoryHost, int);
      SCHURCHEB_MEMCPY( this->_L.GetJ(), L_j, L_i[nLU], kMemoryHost, kMemoryHost, int);
      SCHURCHEB_MEMCPY( this->_L.GetData(), L_data, L_i[nLU], kMemoryHost, kMemoryHost, T);
      
      /* now the remaining part of L, in E */
      this->_E.Setup( m, nLU, L_i[n]-L_i[nLU]);
      
      SCHURCHEB_MEMCPY( this->_E.GetI(), L_i+nLU, m+1, kMemoryHost, kMemoryHost, int);
      SCHURCHEB_MEMCPY( this->_E.GetJ(), L_j+L_i[nLU], L_i[n]-L_i[nLU], kMemoryHost, kMemoryHost, int);
      SCHURCHEB_MEMCPY( this->_E.GetData(), L_data+L_i[nLU], L_i[n]-L_i[nLU], kMemoryHost, kMemoryHost, T);
      ...
      for(i = 0 ; i < nLU ; i ++)
      {
         ...
         SCHURCHEB_MEMCPY( this->_U.GetJ() + this->_U.GetI()[i], U_j + U_i[i], u_end[i] - U_i[i], kMemoryHost, kMemoryHost, int);
         SCHURCHEB_MEMCPY( this->_U.GetData() + this->_U.GetI()[i], U_data + U_i[i], u_end[i] - U_i[i], kMemoryHost, kMemoryHost, T);
         
         SCHURCHEB_MEMCPY( this->_F.GetJ() + this->_F.GetI()[i], U_j + u_end[i], U_i[i+1] - u_end[i], kMemoryHost, kMemoryHost, int);
         SCHURCHEB_MEMCPY( this->_F.GetData() + this->_F.GetI()[i], U_data + u_end[i], U_i[i+1] - u_end[i], kMemoryHost, kMemoryHost, T);
      ...
      /* ready, free */
      SCHURCHEB_FREE( L_i, kMemoryHost);
      SCHURCHEB_FREE( L_j, kMemoryHost);
      SCHURCHEB_FREE( L_data, kMemoryHost);
      SCHURCHEB_FREE( U_i, kMemoryHost);
      SCHURCHEB_FREE( U_j, kMemoryHost);
      SCHURCHEB_FREE( U_data, kMemoryHost);
   ...
   int IluClass<MatrixType, VectorType, DataType>::Solve( VectorType &x, VectorType &rhs)
   {
      ...
      if(this->_option != kIluOptionILUT && this->_option != kIluOptionILUK)
      {
         ...
         SCHURCHEB_ERROR("Solve phase of ILU only supports ILUT yet.");
         return SCHURCHEB_ERROR_FUNCTION_CALL_ERR;
      }
      
      SCHURCHEB_CHKERR(this->_n != x.GetLengthLocal() || this->_n != rhs.GetLengthLocal());
      
      if(!(this->_ready))
      {
         SCHURCHEB_ERROR("Solve without setup.");
         return SCHURCHEB_ERROR_FUNCTION_CALL_ERR;
      }
   ...
   int IluClass<MatrixType, VectorType, DataType>::SolveL( VectorType &x, VectorType &rhs)
   {
      ...
      {
         /* The solve of Partial ILUT is not supported directly, call SolveL and SolveU instead */
         SCHURCHEB_ERROR("Solve with L only supports Partial ILUT yet.");
         return SCHURCHEB_ERROR_FUNCTION_CALL_ERR;
      }
      
      SCHURCHEB_CHKERR(this->_nB != x.GetLengthLocal() || this->_nB != rhs.GetLengthLocal());
   ...
   int IluClass<MatrixType, VectorType, DataType>::SolveU( VectorType &x, VectorType &rhs)
   {
      ...
      {
         /* The solve of Partial ILUT is not supported directly, call SolveL and SolveU instead */
         SCHURCHEB_ERROR("Solve with U only supports Partial ILUT yet.");
         return SCHURCHEB_ERROR_FUNCTION_CALL_ERR;
      }
   ...
   {
      SCHURCHEB_CHKERR(this->_option != kIluOptionPartialILUT);
      SCHURCHEB_CHKERR(this->_x_temp.GetDataLocation() == kMemoryDevice);
   ...
   int IluClass<MatrixType, VectorType, DataType>::SetNB( int nB)
   {
      if(this->_ready)
      {
         SCHURCHEB_ERROR("Change size of B block after Setup is not allowed.");
         return SCHURCHEB_ERROR_FUNCTION_CALL_ERR;
      }
   ...
   int IluClass<MatrixType, VectorType, DataType>::SetOption( int option)
   {
      if(this->_ready)
      {
         SCHURCHEB_ERROR("Change ILU option after Setup is not allowed.");
         return SCHURCHEB_ERROR_FUNCTION_CALL_ERR;
      }
   ...
   int IluClass<MatrixType, VectorType, DataType>::SetOpenMPOption( int omp_option)
   {
      ...
      if(this->_ready)
      {
         if(omp_option != this->_omp_option && SchurchebGetOpenmpGlobalMaxNumThreads() > 1)
         {
            switch(omp_option)
            {
               case kIluOpenMPLevelScheduling:

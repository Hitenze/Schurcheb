#ifndef SCHURCHEB_ILU_H
#define SCHURCHEB_ILU_H
 ...
 * 
 * Modifications are made to fit the Schurcheb data structure
...
namespace schurcheb
{
   ...
   private:
       ...
       */
      typename std::conditional<SchurchebIsDoublePrecision<DataType>::value, 
                                 double, 
	...
	public:
      ...
      static void AbsMinHeapAdd_RN(T1 *heap, T2 *v, int len)
      {
         ...
         while(len > 0)
         {
            ...
            p = (len-1)/2;
            if(SchurchebAbs(heap[-p]) < SchurchebAbs(heap[-len]))
            {
               /* this is smaller */
      ...
      static void AbsMinHeapRemove_RN(T1 *heap, T2 *v, int len)
      {
         ...
         while(l < len)
         {
            ...
            l = r >= len || SchurchebAbs(heap[-l])>SchurchebAbs(heap[-r]) ? l : r;
            if(SchurchebAbs(heap[-l])>SchurchebAbs(heap[-p]))
            {
      ...
      static void Qsplit(T *a, int *ind, int left, int bound, int right)
      {
         ...
         {
            if(SchurchebAbs(a[i]) > SchurchebAbs(a[left]))
            {
      ...
      int         SetLevelOfFill( int lfil)
      {
         if(this->_ready)
         {
            SCHURCHEB_WARNING("Change setting after Setup is not going to change the preconditioner.");
         }

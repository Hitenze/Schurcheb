...
namespace schurcheb
{
   ...
   int CsrMatrixClass<T>::Clear()
   {
      ...
      {
         SCHURCHEB_CUSPARSE_CALL( (cusparseDestroySpMat(this->_cusparse_mat)) );
   ...
   {
      SCHURCHEB_CHKERR( nrows < 0 );
      SCHURCHEB_CHKERR( ncols < 0 );
      SCHURCHEB_CHKERR( nnz < 0 );
      ...
      if(setzero)
      {
         ...
         {
            SCHURCHEB_ERROR("Csr matrix setup setzero option is not availiabe for the device memory.");
   ...
   int CsrMatrixClass<T>::PushBack( int col, T v)
   {
      ...
      SCHURCHEB_CHKERR( (this->_iscsr && (col < 0 || col >= this->_ncols) ) );
      SCHURCHEB_CHKERR( (!(this->_iscsr) && (col < 0 || col >= this->_nrows) ) );

#ifdef SCHURCHEB_CUDA
      if( this->GetDataLocation() == kMemoryDevice)
      {
         SCHURCHEB_ERROR("Csr matrix PushBack function is not availiabe for the device memory.");
         return SCHURCHEB_ERROR_MEMORY_LOCATION;
      }
#endif      
   ...
   int CsrMatrixClass<T>::SubMatrix( vector_int &row_starts, vector_int &col_starts, vector_int &row_ends, vector_int &col_ends, int location, CsrMatrixClass<T> &csrmat_out)
   {
      ...
      if(this->GetDataLocation() == kMemoryDevice)
      {
         SCHURCHEB_ERROR("Extracting csr sub-matrix only availiable on the host memory.");
         return SCHURCHEB_ERROR_MEMORY_LOCATION;
      }
      ...
      {
         SCHURCHEB_ERROR("Working vectors for extracting csr sub-matrix can only be on the host memory.");
         return SCHURCHEB_ERROR_MEMORY_LOCATION;
      }
#endif
      ...
      SCHURCHEB_CHKERR(nblocks != col_starts.GetLengthLocal());
      ...
      SCHURCHEB_CHKERR( nrB > nrA);
      SCHURCHEB_CHKERR( ncB > ncA);
      ...
      for( i = 0 ; i < nblocks ; i ++)
      {
         for(j = col_starts[i] ; j < col_ends[i] ; j ++)
         {
            ...
            SCHURCHEB_CHKERR( j < 0);
            SCHURCHEB_CHKERR( j >= ncA);
            SCHURCHEB_CHKERR( marker[j] != -1);
   ...
   int CsrMatrixClass<T>::SubMatrix( vector_int &rows, vector_int &cols, int location, CsrMatrixClass<T> &csrmat_out)
   {
      ...
      {
         SCHURCHEB_ERROR("Working vectors for extracting csr sub-matrix can be on the host memory.");
         return SCHURCHEB_ERROR_MEMORY_LOCATION;
      }
#endif
   ...
   int CsrMatrixClass<T>::SubMatrixNoPerm( vector_int &rows, vector_int &cols, vector_int &row_perm, vector_int &col_perm, bool complement, int location, CsrMatrixClass<T> &csrmat_out)
   {
      ...
      err = this->SubMatrix(row_perm, col_perm, location, csrmat_out); SCHURCHEB_CHKERR(err);
   ...
   int CsrMatrixClass<T>::GetConnectedComponents(std::vector<vector_int> &comp_indices, int &ncomps)
   {
      ...
      {
         SCHURCHEB_ERROR("Connected components only for square matrices.");
      }
      ...
      if(this->GetDataLocation() == kMemoryDevice)
      {
         SCHURCHEB_ERROR("Extracting connected components only availiable on the host memory.");
         return SCHURCHEB_ERROR_MEMORY_LOCATION;
      }
#endif
   ...
   {
      SCHURCHEB_CHKERR( this->_i_vec.GetDataLocation() != this->_j_vec.GetDataLocation() );
#ifdef SCHURCHEB_DEBUG
      if(this->_isholdingdata)
      {
         SCHURCHEB_CHKERR( this->_i_vec.GetDataLocation() != this->_a_vec.GetDataLocation() );
      }
#endif
   ...
   int CsrMatrixClass<T>::SetNumNonzeros()
   {
      if(this->GetDataLocation() == kMemoryDevice)
      {
         SCHURCHEB_ERROR("Set csr matrix nnz only work on the host memory.");
         return SCHURCHEB_ERROR_MEMORY_LOCATION;
      }
   ...
   int CsrMatrixClass<T>::Eye()
   {
      ...
      SCHURCHEB_CHKERR( this->_ncols != this->_nrows);
      ...
      /* create I, J, and A */
      err = this->_i_vec.UnitPerm(); SCHURCHEB_CHKERR(err);
      err = this->_j_vec.Setup(n); SCHURCHEB_CHKERR(err);
      err = this->_j_vec.UnitPerm(); SCHURCHEB_CHKERR(err);
      err = this->_a_vec.Setup(n); SCHURCHEB_CHKERR(err);
      err = this->_a_vec.Fill(1.0); SCHURCHEB_CHKERR(err);
   ...
   int CsrMatrixClass<T>::Fill(const T &v)
   {
      ...
      err = this->_a_vec.Fill(v); SCHURCHEB_CHKERR(err);
   ...
   {
      SCHURCHEB_CHKERR(!this->IsHoldingData());
   ...
   int CsrMatrixClass<T>::GetDiagScale(T &scale)
   {
      ...
      if(this->GetDataLocation() == kMemoryDevice)
      {
         SCHURCHEB_ERROR("GetDiagScale only work for the host memory.");
         return SCHURCHEB_ERROR_MEMORY_LOCATION;
      }
      ...
      if(this->_iscsr)
      {
         for(i = 0 ; i < this->_nrows ; i ++)
         {
            ...
            for(j = j1 ; j < j2 ; j ++)
            {
               ...
               {
                  /* diagonal! */
                  temp_val = SchurchebAbs(this->_a_vec[j]);
                  if(temp_val > val )
                  {
                     scale = this->_a_vec[j];
                     val = temp_val;
   ...
   {
      SCHURCHEB_ERROR("CSR matrix matmat not supported yet.");
      return SCHURCHEB_SUCCESS;
   }
   ...
   int CsrMatrixClass<T>::Convert(bool csr)
   {
      ...
      if(location == kMemoryDevice)
      {
         SCHURCHEB_ERROR("Convert between csr and csc only works on the host.");
         return SCHURCHEB_ERROR_MEMORY_LOCATION;
      }
      ...
      SCHURCHEB_MALLOC( i_temp, nrows+1, kMemoryHost, int);
      SCHURCHEB_MEMCPY( i_temp, this->GetI(), nrows+1, kMemoryHost, location, int);
      
      SCHURCHEB_MALLOC( j_temp, nnz, kMemoryHost, int);
      SCHURCHEB_MEMCPY( j_temp, this->GetJ(), nnz, kMemoryHost, location, int);
      
      if(hold_data)
      {
         SCHURCHEB_MALLOC( a_temp, nnz, kMemoryHost, T);
         SCHURCHEB_MEMCPY( a_temp, this->GetData(), nnz, kMemoryHost, location, T);
      ...
      SCHURCHEB_FREE(i_temp, kMemoryHost);
      SCHURCHEB_FREE(j_temp, kMemoryHost);
      
      if(hold_data)
      {
         SCHURCHEB_FREE(a_temp, kMemoryHost);
      }
   ...
   int CsrMatrixClass<T>::PlotPattern( int *rperm, int *cperm, int conditiona, int conditionb)
   {
      ...
      if(!(this->_iscsr))
      {
         SCHURCHEB_ERROR("Plot Pattern only works for CSR matrix.");
      }
   ...
   int CsrMatrixClass<T>::Plot( int *perm, int conditiona, int conditionb, int width)
   {
      ...
      if(this->GetDataLocation() == kMemoryDevice)
      {
         SCHURCHEB_ERROR("Plot matrix only availiable on host.");
         return SCHURCHEB_ERROR_MEMORY_LOCATION;
      }
   ...
   int CsrMatrixClass<T>::Laplacian(int nx, int ny, int nz, T alphax, T alphay, T alphaz, T shift, bool rand_perturb)
   {
      ...
      err = coo_mat.Laplacian( nx, ny, nz, alphax, alphay, alphaz, shift, rand_perturb); SCHURCHEB_CHKERR(err);
      err = coo_mat.ToCsr( kMemoryHost, *this); SCHURCHEB_CHKERR(err);
      err = coo_mat.Clear(); SCHURCHEB_CHKERR(err);
   ...
   int CsrMatrixClass<T>::Helmholtz(int n, T w)
   {
      int err;
      CooMatrixClass<T> coo_mat;
      err = coo_mat.Helmholtz( n, w); SCHURCHEB_CHKERR(err);
      err = coo_mat.ToCsr( kMemoryHost, *this); SCHURCHEB_CHKERR(err);
      err = coo_mat.Clear(); SCHURCHEB_CHKERR(err);
   ...
   int CsrMatrixClass<T>::ReadFromMMFile(const char *matfile, int idxin)
   {
      int err;
      CooMatrixClass<T> coo_mat;
      err = coo_mat.ReadFromMMFile( matfile, idxin); SCHURCHEB_CHKERR(err);
      err = coo_mat.ToCsr( kMemoryHost, *this); SCHURCHEB_CHKERR(err);
      err = coo_mat.Clear(); SCHURCHEB_CHKERR(err);

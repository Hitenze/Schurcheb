...
namespace schurcheb
{
   ...
   {
      SCHURCHEB_CHKERR(row < 0);
      SCHURCHEB_CHKERR(col < 0);
      SCHURCHEB_CHKERR(row >= this->_nrows);
      SCHURCHEB_CHKERR(col >= this->_ncols);
   ...
   {
      SCHURCHEB_CHKERR( num_rows < 0);
      SCHURCHEB_CHKERR( num_cols < 0);
      SCHURCHEB_CHKERR( row_start < 0);
      SCHURCHEB_CHKERR( col_start < 0);
      SCHURCHEB_CHKERR( row_start + num_rows > mat_in.GetNumRowsLocal());
      SCHURCHEB_CHKERR( col_start + num_cols > mat_in.GetNumColsLocal());
   ...
   int DenseMatrixClass<T>::SubMatrix( int row_start, int col_start, int num_rows, int num_cols, int location, DenseMatrixClass<T> &mat_out)
   {
      ...
      SCHURCHEB_CHKERR( row_start < 0);
      SCHURCHEB_CHKERR( col_start < 0);
      SCHURCHEB_CHKERR( row_start + num_rows > this->_nrows);
      SCHURCHEB_CHKERR( col_start + num_cols > this->_ncols);
      ...
      if( num_rows == this->_ldim)
      {
         ...
         SCHURCHEB_MEMCPY( data_to, data_from, num_rows*num_cols, location, loc_from, T);
      }
      else
      {
         for (i = 0 ; i < num_cols ; i ++) 
         {
            SCHURCHEB_MEMCPY( data_to, data_from, num_rows, location, loc_from, T);
            data_to += num_rows;
   ...
   {
      SCHURCHEB_CHKERR( this->_nrows != this->_ncols);
   ...
   int DenseMatrixClass<T>::Schur( DenseMatrixClass<T> &Q, SequentialVectorClass<T> &wr, SequentialVectorClass<T> &wi)
   {
      ...
      err = this->Hess(Q1); SCHURCHEB_CHKERR(err);
      err = this->HessSchur(Q2, wr, wi); SCHURCHEB_CHKERR(err);
   ...
   int DenseMatrixClass<T>::Schur( DenseMatrixClass<T> &Q, int start, int end, SequentialVectorClass<T> &wr, SequentialVectorClass<T> &wi)
   {
      ...
      DenseMatrixClass<T> Q1, Q2;
      err = this->Hess(Q1, start, end); SCHURCHEB_CHKERR(err);
      err = this->HessSchur(Q2, start, end, wr, wi); SCHURCHEB_CHKERR(err);
   ...
   int DenseMatrixClass<T>::Schur( DenseMatrixClass<T> &Q, SequentialVectorClass<T> &w)
   {
      ...
      DenseMatrixClass<T> Q1, Q2;
      err = this->Hess(Q1); SCHURCHEB_CHKERR(err);
      err = this->HessSchur(Q2, w); SCHURCHEB_CHKERR(err);
   ...
   int DenseMatrixClass<T>::Schur( DenseMatrixClass<T> &Q, int start, int end, SequentialVectorClass<T> &w)
   {
      ...
      DenseMatrixClass<T> Q1, Q2;
      err = this->Hess(Q1, start, end); SCHURCHEB_CHKERR(err);
      err = this->HessSchur(Q2, start, end, w); SCHURCHEB_CHKERR(err);
   ...
   {
      int err;
      err = this->Schur( QS, wr, wi); SCHURCHEB_CHKERR(err);
      err = this->HessEig( QE, wr, wi); SCHURCHEB_CHKERR(err);
      return err;
   }
   ...
   {
      int err;
      err = this->Schur( QS, w); SCHURCHEB_CHKERR(err);
      err = this->HessEig( QE, w); SCHURCHEB_CHKERR(err);
      return err;
   }
   ...
   int DenseMatrixClass<T>::Plot( int conditiona, int conditionb, int width)
   {
      ...
      {
         SCHURCHEB_ERROR("Plot matrix only availiable on the host memory.");

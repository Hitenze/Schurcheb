#ifndef SCHURCHEB_MATRIXOPS_H
#define SCHURCHEB_MATRIXOPS_H
...
namespace schurcheb
{
   ...
   /**
    * @brief   In place Csr-dense matrix multiplication C = alppha * A * B + beta * C.
    * @details In place Csr-dense matrix multiplication C = alppha * A * B + beta * C. Doesn't support transpose yet.
    * @param [in]       alpha The alpha value.
    * @param [in]       A Csr matrix.
    * @param [in]       B The input dense matrix.
    * @param [in]       beta The beta value.
    * @param [in,out]   C The output dense matrix.
    * @return           Return error message.
    */
   template <typename T>
   ...
   /**
    * @brief   In place parallel Csr-dense matrix multiplication C = alppha * A * B + beta * C.
    * @details In place parallel Csr-dense matrix multiplication C = alppha * A * B + beta * C. Doesn't support transpose yet.
    * @param [in]       alpha The alpha value.
    * @param [in]       A Csr matrix.
    * @param [in]       B The input dense matrix.
    * @param [in]       beta The beta value.
    * @param [in,out]   C The output dense matrix.
    * @param [in]       working Working vector for the communication. Will automatically expand if size not large enough.
    * @return           Return error message.
    */
   template <typename T>
   ...
   /**
    * @brief   DD with edge separator.
    * @details DD with edge sepqrator using METIS. \n
    * @param [in]    A The input CSR matrix.
    * @param [in]    M The second input CSR matrix.
    * @param [in]    n2 The number MPI columns.
    * @param [in]    ndom The number of subdomain.
    * @param [out]   perm The permutation array. A(p,p) and M(p,p) are in the new ordr.
    * @param [out]   dom_ptr Length ndom+1. The pointer for the start of each subdomain. The last value is n.
    * @param [out]   Bdiagsizes Length ndom. The dense diagonal block size of each subdomain, used for shift-invert.
    * @return       Return error message.
    */
   ...
   /**
    * @brief   Get the upper triangular of A and put into B. Matlab notation B = triu(A,0). 
    * @details Get the upper triangular of A and put into B. Matlab notation B = triu(A,0). 
    * @param [in]    A The input CSR matrix.
    * @param [out]   B The out CSR matrix.
    * @return       Return error message.
    */
    ...
    * @return           return SCHURCHEB_SUCESS or error information. 
    */
    ...
    */
   template <class VectorType, class MatrixType, typename DataType, typename RealDataType>
   int SchurchebArnoldiNoRestart( MatrixType &A, int mstart, int msteps, DenseMatrixClass<DataType> &V, DenseMatrixClass<DataType> &H, RealDataType tol_orth, RealDataType tol_reorth, int &nmvs);
    ...
    */
   template <class VectorType, class MatrixType, typename DataType, typename RealDataType>
   int SchurchebArnoldiNoRestart2( MatrixType &A, int mstart, int msteps, DenseMatrixClass<DataType> &V, DenseMatrixClass<DataType> &H, RealDataType tol_orth, RealDataType tol_reorth, DataType shift, int &nmvs);
    ...
    */
   template <class VectorType, class MatrixType, typename DataType, typename RealDataType>
   int SchurchebArnoldiThickRestartNoLock( MatrixType &A, int msteps, int maxits, int rank, int rank2, RealDataType tr_fact, RealDataType tol_eig, char* opt, DenseMatrixClass<DataType> &V, DenseMatrixClass<DataType> &H, RealDataType tol_orth, RealDataType tol_reorth, int &nmvs);
    ...
    */
   template <class VectorType, class MatrixType, typename DataType, typename RealDataType>
   int SchurchebArnoldiThickRestartNoLock( MatrixType &A, int msteps, int maxits, int rank, int rank2, RealDataType tr_fact, RealDataType tol_eig, RealDataType (*weight)(ComplexValueClass<RealDataType>), DenseMatrixClass<DataType> &V, DenseMatrixClass<DataType> &H, RealDataType tol_orth, RealDataType tol_reorth, int &nmvs);
    ...
    */
   template <class VectorType, typename DataType, typename RealDataType>
   int SchurchebCgs2( VectorType &w, DenseMatrixClass<DataType> &V, DenseMatrixClass<DataType> &H, RealDataType &t, int k, RealDataType tol_orth);
    ...
    */
   template <class VectorType, typename DataType, typename RealDataType>
   int SchurchebMgs( VectorType &w, DenseMatrixClass<DataType> &V, DenseMatrixClass<DataType> &H, RealDataType &t, int k, RealDataType tol_orth, RealDataType tol_reorth);
    ...
    */
   template <class VectorType, typename DataType, typename RealDataType>
   int SchurchebOrthogonal( VectorType &w, DenseMatrixClass<DataType> &V, RealDataType &t, int k, RealDataType tol_orth, RealDataType tol_reorth);
   
   /**
    * @brief   Give V and help vector v, put the orthgonal basis of V in W, W should be allocated already.
    * @details Give V and help vector v, put the orthgonal basis of V in W, W should be allocated already. \n
    *          If v is parallel vector, parallel version is applied.
    * @param [in]    V The input matrix.
    * @param [in]    v The help vector.
    * @param [out]   W The output matrix. Space should be allocated outside.
    * @param [out]   n The first n column of W are the basis.
    * @param [in]    tol_orth The tolerance.
    * @return       Return error message.
    */
   template <class VectorType, typename DataType, typename RealDataType>
   ...
   /**
    * @brief   The QR factorization via classical Gram-Schmidt with re-orthgonal (always one step of re-orth).
    * @details The QR factorization via classical Gram-Schmidt with re-orthgonal (always one step of re-orth). \n
    *          This code only works for "tall-thin" matrix. We remove those zeros entries.
    * @param [in]       A Matrix holding the input matrix.
    * @param [in]       v The working vector.
    * @param [in, out]  Q The Q matrix of QR. Space should be allocated.
    * @param [in, out]  R The R matrix of QR. Space should be allocated.
    * @param [in, out]  m Number of columns used in V, on return the actual number of columns in Q.
    * @param [in]       keepR do we compute R? If not we only use R as working vector.
    * @param [in]       tol_orth tol used to check break.
    * @return           return SCHURCHEB_SUCESS or error information.                                                                                                                              
    */
   template <class VectorType, typename DataType, typename RealDataType>
   int SchurchebQR( DenseMatrixClass<DataType> &A, VectorType &v, DenseMatrixClass<DataType> &Q, DenseMatrixClass<DataType> &R, int &m, bool keepR, RealDataType tol_orth);
    ...
    * @param [in]    v The help vector.
    * @param [out]   W The output matrix. Space should be allocated outside.
    * @param [in]    nB The target block size. When nB = 1 this should be equavilent to the standard QR.
    * @param [out]   n The first n column of W are the basis.
    * @param [in]    tol_orth The tolerance.
    * @return       Return error message.
    */
    ...
    * @param [in]       bj_last  Should we treat last level with block Jacobi?
    * @param [in]       parallel_log Parallel info struct.
    * @return           return SCHURCHEB_SUCCESS or error information.
    * @note             We assume that A_mat has symmetric pattern.
    */

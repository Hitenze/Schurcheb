...
namespace schurcheb
{
   ...
   {
      return this->Setup( nrows, ncols, SchurchebMax( nrows, ncols)*schurcheb::SCHURCHEB_global::_coo_reserve_fact, kMemoryHost);
   }
   ...
   int CooMatrixClass<T>::PushBack( int row, int col, T v)
   {
      ...
      SCHURCHEB_CHKERR( row < 0 || row >= this->_nrows);
      //SCHURCHEB_CHKERR( col < 0 || col >= this->_ncols);
